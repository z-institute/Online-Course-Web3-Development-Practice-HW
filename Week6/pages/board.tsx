...
const Board: NextPage = () => {
  ...
  let [message, setMessage] = useState<string>("Input Message");
  let [value, setValue] = useState<number>(1);
  let { balance } = useTokenBalance();
  let { allowance } = useTokenAllowence();

  //   let newAnnouncementMyToken = () => {};
  let { newAnnouncementPoToken, status: newAnnouncementStatus } = useBoardNewAnnouncement(
    message,
    value
  );
