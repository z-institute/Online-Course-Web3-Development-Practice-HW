...
const Card = ({ cardData, cardBgColor, colors }: cardProps) => {
  ...
  const getLastName = (str: String | undefined | null) => { return (str || '').split("/").slice(-1)[0] }
  ...
  // console.log("cardData github",cardData.github);
        ...
        <Flex direction="column" w="100%" h='100%'>
          <Flex flex={5} direction="column">
            <Flex flex={3} direction="column" pr="5">
              <Text color={convertColor(colors[4]) || 'brand'}>Stay Hungry, Stay Foolish</Text>
              <Spacer></Spacer>
              <Box>
                {/* <Heading>Che-Yu Wu</Heading> */}
                {cardData.avatarUrl && <Image my={3} boxSize="150px" src={cardData.avatarUrl} alt="" />}
                <Heading style={{ fontSize:32 }}>{cardData.ensName}</Heading>
          ...
          <Flex flex={1}>
            {// @ts-ignore
              <Box fontSize="7xl" mb={-5} style={{ color: 'transparent', fontWeight: 900 }}>
                {cardData.ensName}
              </Box>
            }
          </Flex>
